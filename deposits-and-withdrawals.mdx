...
- **Direct Mint/Redeem Tranches.** Deposit the accepted asset into any individual ST, SLP, or JT market through the RoycoEntryPoint and receive the corresponding tranche token, or redeem it for the strategy base asset. ST can exit through the SLP, or has a primary exit option if a market hits its **Protected Exit Threshold**. If JT’s full balance is backing ST exposure (utilization at 100%), JT withdrawals are restricted until additional JT capital enters or ST capital exits. During an Observation Period, ST deposits and redemptions, JT deposits and redemptions, and SLP redemptions are blocked. SLP deposits remain available; multi-asset SLP deposits can only use the quote leg while ST deposits are disabled.
...
Senior and Junior redemptions always exit in-kind. The SLP tranche can choose how to exit:

- **In-kind:** receive the LP position and any accrued senior shares directly.
- **Multi-asset:** unwind the LP position into its underlying constituents plus the quote asset.
- **Optimized:** the market takes whichever of the two routes fills more of the request.
